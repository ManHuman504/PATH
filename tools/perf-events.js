...
async function run() {
  console.log(`[perf-events] Publishing ${total} events via commands...`);
  const start = Date.now();

  for (let i = 0; i < total; i += batchSize) {
    const batch = Array.from({ length: Math.min(batchSize, total - i) }, (_, idx) => {
      const title = `Perf Path ${i + idx + 1}`;
  ...
  console.log(`[perf-events] Completed in ${elapsed} ms`);
}

run().catch((error) => {
  console.error('[perf-events] Failed:', error.message);
  process.exit(1);
});
